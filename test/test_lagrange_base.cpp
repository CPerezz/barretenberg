...
TEST(lagrange_base, verify_lagrange_base_import_srs)
{
    ...
    // step 4: commit to poly over both reference strings
    // step 5: very correctness
    waffle::ReferenceString reference_string(degree);

    std::vector<g1::affine_element> monomials
    std::vector<g1::affine_element> lagrange_base_srs;
    lagrange_base::transform_srs(&r[0], &lagrange_base_srs[0], degree);
    ...
    g1::element expected =
        scalar_multiplication::pippenger(&test_polynomial[0], &reference_string.monomials[0], degree);
    g1::element result =
        scalar_multiplication::pippenger(&lagrange_base_polynomial[0], &reference_string.lagrange[0], degree);
    expected = g1::normalize(expected);
    result = g1::normalize(result);
